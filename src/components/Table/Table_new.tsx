...
export const Table = <T extends Record<string, any>>({
  ...
  const handleSelectRow = (recordKey: string | number, record: T) => {
    const newSelectedKeys = currentSelectedKeys.includes(recordKey)
      ? currentSelectedKeys.filter(key => key !== recordKey)
      : [...currentSelectedKeys, recordKey];
  ...
  const rowStyles = (index: number, isSelected: boolean = false): React.CSSProperties => ({
    ...
    backgroundColor: isSelected 
      ? tableTheme.row.backgroundSelected
      : striped && index % 2 === 1 
        ? tableTheme.row.backgroundAlt 
        : tableTheme.row.background,
                ...
                {selectable && (
                  <td style={{ ...cellStyles('center'), width: '40px' }}>
                    <input
                      type="checkbox"
                      checked={isSelected}
                      onChange={() => handleSelectRow(recordKey, record)}
                      style={checkboxStyles}
