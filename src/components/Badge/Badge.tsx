...
export const Badge: React.FC<BadgeProps> = ({
  ...
  const formatContent = () => {
    ...
    const numericValue = typeof children === 'string' ? parseInt(children, 10) : 
  ...
  return (
    <span
      className={className}
      style={baseStyles}
      role={style === 'dot' ? 'status' : undefined}
      aria-label={style === 'dot' ? `${variant} status indicator` : undefined}
    >
      {style !== 'dot' && content}
    </span>
  );
